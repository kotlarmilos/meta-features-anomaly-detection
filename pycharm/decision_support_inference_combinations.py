...
characterization_attribute="simple" #simple
...
for characterization_filter in characterization_filters:
    ...
    columns_to_combine = []
    if characterization_attribute == 'predefined':
        columns_to_combine = predefined_characterization_columns
        # idx = np.in1d(characterization_columns, predefined_characterization_columns).nonzero()[0]
        # features = mid_features.to_numpy()[:, idx]
    elif characterization_attribute == 'simple':
        columns_to_combine = simple
        # idx = np.in1d(characterization_columns, simple).nonzero()[0]
        # features = mid_features.to_numpy()[:, idx]

    for comb_k in range(1, 6):
        combs = combinations(columns_to_combine, comb_k)
        comb_counter = 0
        for comb in combs:
            idx = np.in1d(columns_to_combine, np.array(comb)).nonzero()[0]
            features = mid_features.to_numpy()[:, idx]
            comb_counter +=1
            if comb_counter>1000:
                break
        ...
        #         features = mid_features.to_numpy()
            ...
            for eval_method in methods:
                if eval_method['name'] not in ['Eucledian']:  # ''#'',,'AutoencoderModel', 'Gaussian', 'Linear', 'Manhattan', 'RPCA', 'KMeans'
                    continue
                ...
                # if (method_match[0]>0.8):
                print(comb, method_match[0], characterization_filter)
                summary_results.append((comb, method_match[0], characterization_filter))
